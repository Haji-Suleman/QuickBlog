...
const Header = () => {
    ...
    const inputRef = useRef<HTMLInputElement>(null); // ✅ Correct ref type for input element

    const onSubmitHandler = (e: FormEvent) => { // ✅ Explicit type for event
        e.preventDefault();
        if (inputRef.current) {
            setInput(inputRef.current.value); // ✅ Type-safe access
        }
    };
    ...
    const onClear = () => {
        setInput('');
        if (inputRef.current) {
            inputRef.current.value = ''; // ✅ Type-safe access
        }
    };
